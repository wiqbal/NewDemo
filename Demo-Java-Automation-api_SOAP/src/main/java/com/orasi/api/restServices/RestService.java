...
public class RestService {	
	...
	private RestResponse sendRequest(HttpUriRequest request){
        ...
        if (request.getURI().toString().toLowerCase().contains("https")) {
            sslHttpClient = TrustedSSLContext.buildHttpClient(timeout);
        }
        RestResponse response = null;
        try (CloseableHttpClient httpClient = (sslHttpClient == null ? HttpClientBuilder.create().setDefaultRequestConfig(requestConfig).build() : sslHttpClient)) {
           ...
           String executionTime = execution.toString();
            ...
            TestReporter.logDebug("Failed to establish connection after [ " + timeout + " ] seconds: " + t1.getMessage());
            throw new WebServiceException("Failed to establish connection after [ " + timeout + " ] seconds", t1);
        } catch (IOException e) {
            TestReporter.logDebug("Failed to send request: " + e.getMessage());
            throw new WebServiceException("Failed to send request", e);
        }
